...
RC BplusTreeHandler::drop()
{
  ...
  if (disk_buffer_pool_ != nullptr) {
    BufferPoolManager &bpm       = BufferPoolManager::instance();
    rc                           = bpm.drop_file(disk_buffer_pool_->get_file_name().c_str());
    if (rc != RC::SUCCESS) {
      LOG_ERROR("Failed to drop disk buffer pool of data file. file name=%s", disk_buffer_pool_->get_file_name().c_str());
      return rc;
    }
